...
def train_and_evaluate_fold(args, train_loader, val_loader, test_loader, fold_idx, verbose=False):
    ...
    def get_conv_class(name: str):
        ...
        cls = getattr(tgnn, name, None)
        if cls is None:
            raise ValueError(f"Unknown conv class: {name}")
        return cls
    ...
    elif args.model == "HGPGCNConv":
        # 组合分支（GCN + HypergraphConv），这个类内部已经写死两种conv，一般不需要额外传 gnn
        model = ResidualCombineGNNs(
            args, train_loader.dataset, args.hidden, args.hidden_mlp, args.num_layers
        ).to(args.device)

    else:
        # 纯图分支（GINConv / ChebConv / GCNConv 等）
        gnn_class = get_conv_class(args.model)  # 例如 "GINConv" -> torch_geometric.nn.GINConv
        model = ResidualGNNs(
            args, train_loader.dataset, args.hidden, args.hidden_mlp, args.num_layers, gnn_class
        ).to(args.device)
...
def predefined_5fold_cross_validation(args, verbose=False):
    ...
    for fold_idx, fold_info in enumerate(fold_combinations):
        ...
        try:
            ...
            train_data, val_data, test_data, train_labels, val_labels, test_labels = load_data_with_fold_split_combine(args, fold_info)
