...
def train_and_evaluate_fold(args, train_loader, val_loader, test_loader, fold_idx, verbose=False):
    ...
    elif args.model == "tHGCNConv":
        model = timeResidualHyperGNNs(
            args, train_loader.dataset, args.hidden, args.hidden_mlp, args.num_layers
        ).to(args.device)
    else:
...
def predefined_5fold_cross_validation(args, verbose=False):
    ...
    for fold_idx, fold_info in enumerate(fold_combinations):
        ...
        try:
            ...
            train_data, val_data, test_data, train_labels, val_labels, test_labels = load_data_with_fold_split_ts(args, fold_info)
...
if __name__ == "__main__":
    ...
    # Debug options: set DEBUG_SINGLE = True to run a single args configuration without multiprocessing
    # This is useful for using pdb inside model.forward/other functions.
    DEBUG_SINGLE = True
    DEBUG_ARGS_INDEX = 0            # 挑一个配置
    DEBUG_USE_PREDEFINED_FOLDS = True
    DEBUG_FORCE_CPU = False

    if DEBUG_SINGLE:
        # Run one experiment directly (no Pool) so pdb.set_trace() will stop execution
        args = args_list_a[DEBUG_ARGS_INDEX]
        print(f"[DEBUG] Running single experiment {args.tune_name} on {args.device}")
        # Call the single-run function directly
        run_single_experiment(args, use_predefined_folds=DEBUG_USE_PREDEFINED_FOLDS)
    else:
