...
def train_and_evaluate_fold(args, train_loader, val_loader, test_loader, fold_idx, verbose=False):
    ...
    if args.model == "HGCNConv":
        model = HGNN(in_ch=train_loader.dataset[0].num_features,n_class=args.num_classes,n_hid=args.hidden).to(args.device)
...
argsDictTune_a = {
    'dataset' : ['ADHD'],
    'mode' : ["brainsuite_hg"],
    'timeseries_dir': "/data/public_dataset/ADHD_parcellation/brainsuite_pkl",
